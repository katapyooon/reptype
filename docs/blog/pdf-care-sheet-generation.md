...
reptype は、いくつかの質問に答えると自分に合った爬虫類を提案してくれるWebサービス。診断結果画面ではおすすめの爬虫類とその特徴を表示しているが、これを**印刷・保存して手元に置いておける「ケアシート」PDF**として出力できるようにした。
...
reptype にはもともと、S3に保存した爬虫類ドキュメントを Bedrock の埋め込みモデルでベクトル化し、pgvector で類似検索する RAG の仕組み（チャット機能）がある。PDF生成でもこの資産をそのまま流用し、「セクションごとにRAG検索→Bedrockで文章生成→Markdown的な記法をHTMLへ整形→HTMLをGroverでPDF化」という流れにした。
...
セクション生成部分（`generate_sections`）はプレビュー画面とPDF生成の両方から共通で呼ばれる作りになっている。

## 3. 実装のポイント

### 3.1 GroverでHTML→PDF変換する基盤を作る

PDF生成には [Grover](https://github.com/Studiosity/grover)（Puppeteer/Headless ChromeでHTMLをPDF化するgem）を採用した。Rails側のビュー（ERB）をそのままPDFのレイアウトとして描画できるのが決め手。

コンテナ環境で動かす都合上、Dockerfile側で Node.js と Chromium を同梱し、Puppeteer自体のChromiumダウンロードはスキップして、OSパッケージのChromiumを指す形にしている。
...
`fonts-noto-cjk` を入れているのは、日本語フォントが無いとPDF上で文字が豆腐（□）になってしまうため。[TODO] ここでハマった点があれば追記（フォント抜けで文字化けした画面など）

出力設定（A4・余白・余白なしのプレビューヘッダ抑制など）は `config/initializers/grover.rb` にまとめている。
...
`--no-sandbox` はコンテナ内でChromiumを起動するための定番設定。[TODO] 本番相当環境でのサンドボックス運用について一言添えるか検討。

### 3.2 RAGで爬虫類ごとの飼育情報を5セクション生成する

PDFの各セクション（基本情報／ケージサイズ／設備／餌／温湿度）は、それぞれ専用のクエリでRAG検索した上で、セクションごとに異なるプロンプトでBedrockに文章化させている。
...
最初は基本情報（上段）だけの実装からスタートし、ケージサイズ・設備（下段左）→餌（下段右）→温湿度（下段右）と、PRを分けて段階的にセクションを追加していった。各セクションで「箇条書きの形式を固定する」「タイトル行を含めない」など、プロンプト側でかなり細かく出力フォーマットを指定しているのがポイントで、ここを詰めないとPDFのレイアウトが安定しない。
...
とはいえプロンプトだけで完全に制御しきれるわけではなく、実際には「参考1」のような残骸や断り書きが混じることがあったため、後述のとおりアプリケーション側でもノイズ除去を行っている。

### 3.3 Bedrock呼び出しを並列化してレイテンシを削減

セクションが5つに増えると、Bedrockへの呼び出しを直列に行った場合、単純計算で1回あたりの応答時間の5倍待つことになる。RailsではすでにActiveJob等の依存で `concurrent-ruby` が使えるため、新しいgemを追加せずに `Concurrent::Future` で5並列化した。
...
`f.value!` を使うことで、Future内で例外が起きた場合にそれを呼び出し元まで伝播させている（`value` だと例外が握りつぶされて `nil` になってしまう）。合計の待ち時間が「直列5回分」から「並列1回分（最も遅い呼び出し）」に短縮された。

[TODO] Before/Afterの実測値（例: 直列◯秒→並列◯秒）を入れると説得力が増す

### 3.4 AIの出力をPDF用HTMLに整形する（Markdown風記法→HTML、ノイズ除去）

Bedrockの応答は完全な自由文ではなく、プロンプトで `**太字**` や `•` 箇条書きなど軽量なMarkdown風の記法を使うよう指示しているため、それをPDF/プレビュー画面用のHTMLへ変換する `format_for_pdf` を用意した。あわせて、プロンプトで抑制しきれなかった「参考1」等の残骸行を正規表現で除去している。
...
ここは後述のセキュリティ修正で処理順序を変更しているので、詳細は「4. セキュリティ対応」で扱う。

### 3.5 1ページに収まるようレイアウトを詰める

セクションが増えるにつれて、PDFが2ページ目にはみ出す・画像がカードからはみ出すといった見た目の崩れが発生した。テンプレート側の高さをA4の1123pxに固定して `overflow: hidden` とし、余白・フォントサイズを全体的に縮小することで1ページ厳守のレイアウトにしている。[TODO] Before/Afterのレイアウト崩れスクショがあると分かりやすい

### 3.6 プレビュー画面を追加してPDF生成前に内容を確認できるようにする

PDFはダウンロードするまで中身が見えないため、「生成 → 中身がイマイチ → もう一度」のたびにダウンロードが発生するのは体験として重い。そこで `generate_pdf` から `generate_sections` を切り出し、PDF生成とプレビュー画面（`pdf_preview` アクション）の両方から共通で使えるようにした。
...
診断結果画面（`show.html.erb`）に「もっと詳しく見る」リンクを追加し、別タブでプレビュー画面を開き、そこから「PDFダウンロード」できる導線にした。

[TODO] プレビュー画面のスクリーンショット

## 4. セキュリティ対応

実装を進める中で、AIの出力とユーザー由来のデータをHTML化してPDFに埋め込むという構造上、2つの脆弱性が見つかり修正した。

### 4.1 AI出力によるHTMLインジェクション

`format_for_pdf` は当初、Bedrockの応答テキストに対して直接 `#見出し` → `<strong>`、`**太字**` → `<strong>` の正規表現置換をかけてから `raw` でHTMLに埋め込んでいた。これはつまり、Bedrockの応答に `<script>` のような任意のHTMLタグが含まれていた場合、そのままPDF/プレビュー画面にrawで展開されてしまうということを意味する。

修正は、**HTMLエスケープを先に行ってから、エスケープ済みのテキストに対してMarkdown風記法の変換をかける**という順序に変えるだけ。
...
生成AIの出力を「信頼できない外部入力」として扱う必要がある、という典型例。プロンプトで出力形式を指定していても、それを実行時の安全性の担保にはできない。

### 4.2 画像パスのパストラバーサル

爬虫類の画像は `@type.image_path` を `app/assets/images` 配下から読み込んでBase64化しているが、`image_path` に `../../config/database.yml` のようなパス区切りを含む値が渡された場合、アプリ内の任意ファイルを読み込んでPDFに埋め込めてしまう可能性があった（`image_path` 自体は管理データ由来で外部から直接書き換えられる値ではないが、多層防御として対処）。
...
なお、最初の修正では `basename` だけでファイルパスを組み立てていたため、実際の画像がサブディレクトリ配下にある場合に読み込めなくなる副作用が出た。最終的には「`basename` はバリデーション用途にのみ使い、実際のパス組み立ては元の `image_path`（サブディレクトリを含む）を使う。ただし `realpath` チェックで `images_dir` 配下から出ないことは保証する」という形に落ち着いている。安全性を担保しつつ既存の運用（サブディレクトリ管理）を壊さないための調整、というエピソードとして書ける。

[TODO] このあたり、単体テストでどう担保しているか（あれば）触れる

## 5. その他

今回のPDF生成の開発期間中、副次的に **Ingestion（ドキュメント取り込み）の自動化** も行った。`reptype-chat/docs` 配下のドキュメントを更新してmainにマージすると、GitHub Actionsが自動でS3へ同期し、ECS Fargate上でRailsの `ingestion:run_all` タスクを実行して埋め込みを再生成するワークフローを追加した。
...
これにより、PDFの各セクションが参照するRAGの元データ（爬虫類の飼育ドキュメント）を更新した際に、手動でIngestionを叩き直す手間がなくなった。PDF生成機能そのものではないが、RAGの鮮度を保つという意味では地味に効いている改善。

[TODO] このセクションは軽めのトーンでOK。詳細な運用フローは別記事に切り出すのも手。
